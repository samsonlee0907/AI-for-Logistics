...
The embedded Foundry/Azure OpenAI deployment generates a concise structured recommendation from precomputed facts and citations through the Container App managed identity. It can also return two bounded signals: market and disruption, each between -150 and +150 basis points. The server validates them, clamps their combined impact to +/-300 basis points, applies that amount to the deterministic baseline, and recalculates guardrails. It cannot set a free-form price, change deterministic inputs, invent citations, or bypass approval requirements. If the response is unavailable or invalid, the portal keeps the deterministic baseline and makes the status explicit.
